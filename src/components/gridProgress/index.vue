<template>
  <div :id="svgId" class="grid-progress-grid">
    ...
    <div class="grid-progress-content" ref="content">
      <svg width="100%" :height="option.grid.height" :id="`grid-progress-svg-${svgId}`">
        <defs>
          <clipPath :id="`grid-progress-barcut-${svgId}`">
            <rect y="0" :rx="option.grid.radius" :ry="option.grid.radius" :height="option.grid.height"
              v-for="(item, index) in rxs" :x="item" :key="index" :width="realWidth" />
          </clipPath>
        ...
        <rect x="0" y="0" width="100%" :height="option.grid.height" :fill="option.grid.backgroundColor"
          :clip-path="`url(#grid-progress-bgcut-${svgId})`" />
        <rect x="0" y="0" :width="`${svgRectWidth}px`" :height="option.grid.height"
          :fill="`url(#grid-progress-g1-${svgId})`" :clip-path="`url(#grid-progress-barcut-${svgId})`" />
      </svg>
      <div class="grid-progress-line" :style="{
        top: `${option.grid.height + 5}px`,
        width: `${svgRectWidth}px`,
        background: option.grid.underline ? 'rgba(230,247,255,.6)' : 'rgba(1,1,1,0)',
      }"></div>
      <div :style="{
        ...assistWrapStyle,
        paddingLeft: `${svgRectWidth - option.assist.width / 2}px`,
      }" class="grid-progress-assist">
        <div :style="assistBg">
          <div class="grid-progress-text" :style="textStyle" style="width: 69px">{{ textValue }}</div>
        </div>
      </div>
      <div v-if="option.text.realValue" class="grid-progress-text-label" :style="{
        left: '0px',
      ...
      <div v-if="option.text.realValue" class="grid-progress-text-label" :style="{
        ...
        transform: 'translate(50%, 0px)',
        ...labelStyle,
      }">{{ option.text.showUnit ? data.max + '' + option.text.unit : data.max }}</div>
...
export default {
  ...
  watch: {
    ...
    option: {
      handler() {
        this.getTextValue();
        const show = this.option.animation.show && this.option.animation.dynamicEffect;
        if (!show) return;
        if (this.reqId) {
